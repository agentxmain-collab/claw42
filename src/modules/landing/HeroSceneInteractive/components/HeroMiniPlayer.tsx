    ...
    <AnimatePresence>
      {open && (
        <motion.aside
          ...
          aria-modal="false"
          aria-label={
            data ? `${data.displayName} ${t.hero.miniPlayer.title}` : t.hero.miniPlayer.loading
          }
          initial={reduceMotion ? { opacity: 1 } : { opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 18 }}
          transition={reduceMotion ? { duration: 0 } : { duration: 0.24, ease: "easeOut" }}
          className="border-white/12 bg-[#050b12]/96 fixed inset-x-3 bottom-3 z-[70] max-h-[78vh] overflow-y-auto rounded-lg border p-4 shadow-[0_18px_70px_rgba(0,0,0,0.62)] backdrop-blur-xl md:absolute md:bottom-[13%] md:left-auto md:right-[6%] md:max-h-[64vh] md:w-[380px]"
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              onClick={onClose}
              className="border-white/12 flex h-9 w-9 shrink-0 items-center justify-center rounded-md border bg-white/[0.06] text-xl leading-none text-white transition-colors hover:bg-white/[0.12]"
            >
              ×
            </button>
          </div>
