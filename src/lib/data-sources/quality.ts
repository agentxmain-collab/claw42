...
export function scoreDataSourceQuality(
  ...
  const score = roundScore(
    priceCoverage.score +
      priceFreshness.score +
      volumeChangeCoverage.score +
      newsCoverage.score +
      warningRate.score,
  );
...
export function summarizeStabilityRuns(runs: StabilityRun[]): StabilitySummary {
  ...
  const averageScore = totalRuns
    ? roundScore(runs.reduce((sum, run) => sum + run.quality.score, 0) / totalRuns)
    : 0;
  ...
  return {
    status:
      passRate >= 0.95 && averageScore >= 85
        ? "pass"
        : passRate >= 0.8 && averageScore >= 70
          ? "warn"
          : "fail",
    totalRuns,
    passedRuns,
...
function scorePriceCoverage(
  result: SignalDataSourceResult,
  requiredSymbols: string[],
): DataSourceQualityDimension {
  const covered = requiredSymbols.filter((symbol) =>
    result.priceSnapshots.some((item) => item.symbol === symbol && item.price > 0),
  );
...
function scoreVolumeChangeCoverage(
  result: SignalDataSourceResult,
  requiredSymbols: string[],
): DataSourceQualityDimension {
  const covered = requiredSymbols.filter((symbol) => {
...
function scoreNewsCoverage(result: SignalDataSourceResult): DataSourceQualityDimension {
  const validNews = result.newsItems.filter(
    (item) => item.title.zh && item.title.en && item.summary.zh && item.summary.en,
  );
