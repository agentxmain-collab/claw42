...
async function parseRssItems(xml: string, sourceUrl: string, limit: number, translator?: NewsTranslator) {
  ...
  return Promise.all(itemBlocks.slice(0, limit).map(async (block, index) => {
...
function extractTag(xml: string, parentOrTag: string, maybeTag?: string) {
  if (!maybeTag) {
    const match = xml.match(new RegExp(`<${parentOrTag}\\b[^>]*>([\\s\\S]*?)<\\/${parentOrTag}>`, "i"));
    return match?.[1] ?? null;
  }
...
function inferDirection(text: string): MarketDirection {
  ...
  if (/(hack|exploit|outflow|falls?|drops?|selloff|lawsuit|rejected)/i.test(normalized)) return "bearish";
  if (/(rises?|rally|inflow|approval|improves?|surges?|breakout|demand)/i.test(normalized)) return "bullish";
  return "neutral";
}

function inferSeverity(text: string): Severity {
  return /(etf|fed|sec|hack|exploit|lawsuit|bitcoin|ethereum|solana|btc|eth|sol)/i.test(text) ? "high" : "medium";
}
...
function slugify(value: string) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 40) || "market-update";
}
