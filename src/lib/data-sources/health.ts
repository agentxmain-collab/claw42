...
export async function isDataSourceInCooldown(source: DataSourceName, options: { stateDir?: string; now?: Date } = {}) {
...
export async function recordDataSourceSuccess(source: DataSourceName, options: { stateDir?: string; now?: Date } = {}) {
...
export async function recordDataSourceError(source: DataSourceName, error: unknown, options: { stateDir?: string; cooldownMs?: number; now?: Date } = {}) {
  if (!options.stateDir) return;
...
export async function recordDataSourceFallback(source: DataSourceName, options: { stateDir?: string } = {}) {
  if (!options.stateDir) return;
  await updateHealthFile(source, options.stateDir, (entry) => ({
    ...entry,
...
async function updateHealthFile(source: DataSourceName, stateDir: string, updater: (entry: DataSourceHealthEntry) => DataSourceHealthEntry) {
...
async function readHealthFile(stateDir: string): Promise<DataSourceHealthFile> {
  try {
    const parsed = JSON.parse(await readFile(join(resolveStateDir(stateDir), dataSourceHealthFileName), "utf8")) as DataSourceHealthFile;
