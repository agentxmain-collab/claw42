...
export type DistributionPolicyReason =
  | "health_blocked"
  | "stale_data"
  | "no_major_event"
  | "thin_evidence"
...
export function resolveDistributionPolicy({
  health,
  majorEvent,
}: {
  health: SignalHealth;
  majorEvent: MajorEventAnalysis;
}): DistributionPolicyDecision {
  const reasons: DistributionPolicyReason[] = [];
  const freshnessCheck = health.checks.find((check) => check.key === "freshness");

  if (
    health.status === "blocked" ||
    health.distributionMode === "hold" ||
    freshnessCheck?.status === "fail"
  ) {
    return {
      mode: "hold",
      surface: "none",
      ...
      reasons: [
        health.status === "blocked" || health.distributionMode === "hold"
          ? "health_blocked"
          : "stale_data",
      ],
    };
  ...
  if (majorEvent.evidence.length < 2 || !event.evidence.multiSourceConfirm)
    reasons.push("thin_evidence");
  if (event.judgment.confidence < 40 || event.judgment.direction === null)
    reasons.push("low_confidence");
  if (freshnessCheck?.status === "warn") reasons.push("stale_data");

  const mode: SignalDistributionMode =
    reasons.length === 0 && health.distributionMode === "auto" ? "auto" : "watch_only";
