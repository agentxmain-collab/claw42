...
export function getStructuringProvider(env: EnvLike = process.env): StructuringProvider {
  if (env.SIGNAL_PROVIDER?.trim() === "llm") {
    ...
    console.warn("SIGNAL_PROVIDER=llm is pending T2; falling back to stub provider in non-production.");
  }
  return stubStructuringProvider;
}

export { stubStructuringProvider };
export type { StructuredFields, StructuringProvider, StructuringProviderInput } from "@/lib/signal-engine/providers/types";
