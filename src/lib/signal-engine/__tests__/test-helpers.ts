...
type SignalOverrides = Partial<Omit<SignalCard, "facts" | "explanation" | "judgment" | "impact" | "evidence" | "engine">> & {
...
export function makeNewsItem(overrides: Partial<NewsItem> = {}): NewsItem {
  return {
    ...
    fullSummary: { zh: "现货 ETF 连续流入，成交量同步放大。", en: "Spot ETF inflows extended while volume expanded." },
    ...
    impactedAssets: [
      { symbol: "BTC", direction: "bullish", severity: "high", note: { zh: "ETF 资金确认", en: "ETF flow confirmation" } }
    ],
    timeline: [
      { id: "tl-1", datetime: "2026-04-19T08:30:00.000Z", title: { zh: "ETF 数据更新", en: "ETF data updated" }, detail: { zh: "买盘改善", en: "Demand improved" } }
    ],
    ...overrides
  };
}
