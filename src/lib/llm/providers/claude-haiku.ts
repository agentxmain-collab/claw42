...
const INPUT_USD_PER_MILLION = 0.8;
const OUTPUT_USD_PER_MILLION = 4;

export const claudeHaikuProvider: LLMProvider = {
  id: "claude-haiku",
  ...
  async generate(input: LLMInput): Promise<LLMOutput> {
    ...
    if (!response.ok) throw new Error(`claude-haiku ${response.status}`);
    const data = (await response.json()) as ClaudeResponse;
    const text = data.content?.find((item) => item.type === "text")?.text?.trim();
    if (!text) throw new Error("claude-haiku empty response");
