...
function resolutionContent(
  ...
  const vars = {
    ...
    reasonText: resolution.reason ? t(`agentWatch.dispatchV10.outcome.reason.${resolution.reason}`) : "",
  };
...
*v1.0 起草：2026-05-14 22:30 CST F session*
*起草前置：Codex grep 实测 7 文件完成 / spec 不留 audit trail / 字段名实测准确*
*下一动作：派 Codex 第 1 轮双脑评估 + implement 合并*
