...
function makeMessages(rec: PublicTimelineEvent, ctx: V9AdapterContext): DispatchMessage[] {
  ...
  const stage1Agents: TeamMemberId[] = [
    "fundamental_analyst",
    "news_analyst",
    "chart_analyst",
    "onchain_analyst",
  ...
  const stages: DispatchStageMarker[] = [
    {
      id: "stage-1",
      label: "阶段 1 · 信息收集",
      status: hasAllAnalysts(rationaleByMember)
        ? "done"
        : hasAnyAnalyst(rationaleByMember)
          ? "active"
          : "pending",
    },
    {
      id: "stage-2",
      label: "阶段 2 · 多空辩论",
      status: has("research_lead") ? "done" : "pending",
    },
    ...
    { id: "stage-4", label: "阶段 4 · 风险审查", status: has("risk_lead") ? "done" : "pending" },
    {
      id: "stage-5",
      label: "阶段 5 · 最终决策",
      status: tradeDecision?.direction ? "final" : "pending",
    },
...
export function calculateIntensity(
  ...
  const evidenceList =
    (rec.evidenceIds ?? rec.payload.citationsByMember) ? collectEvidenceIds(rec) : [];
...
function makeStrategy(rec: PublicTimelineEvent, ctx: V9AdapterContext): DispatchStrategy {
  ...
  const followStat = ctx.followStats[rec.payload.recordId] ?? {
    watchCount: 0,
    followCount: 0,
    userFollowed: false,
  };
