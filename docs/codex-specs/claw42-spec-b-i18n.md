...
### 新建
- `middleware.ts`（项目根目录，**不是** `src/middleware.ts`）
...
放**项目根目录**（和 `package.json` 同级的 `middleware.ts`，不是 `src/middleware.ts`）：

```ts
import { NextResponse, type NextRequest } from "next/server";
import { LOCALES, DEFAULT_LOCALE, matchLocale } from "./src/i18n/locales";
...
注意：middleware 从 `./src/i18n/locales` 导入。Next.js 14 允许 middleware 在 src 外或内，放项目根目录是官方推荐位置。
...
- [ ] `ls middleware.ts` 存在；`ls src/middleware.ts` 不存在
