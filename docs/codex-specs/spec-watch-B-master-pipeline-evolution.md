# spec-watch-B-master-pipeline-evolution (v1.1 · 2026-05-15)

> **B 模块 master 路线图 spec** — 把 watch dispatch console 后端从"7 real TeamMember + 12 DispatchAgent / 单 round / fixture-only / polling / no follow-trade"演进到"12 real TeamMember / multi-round trace / streaming / SSE / follow-trade mock+real / writer 全闭环"。
>
...
> **核心范围**：
>
> - **B.2** 12 真 TeamMember dispatch upgrade（**7→12 真升级**：把 5 个 synthetic dispatch role 升级为 real TeamMember，含 LLM prompt / role / desc / provider；memory_loop 同时升级）
> - **B.3** Multi-round debate trace（schema v2 + structured rounds array + 每 stage 多 round）
...
> - **B.9** Post-trade review writeback（manual_close real writer + **加 `manual_close_requested` reason union**；显式解冻 B.1 冻结的 writer 文件）
>
> **不在范围**：
>
...
> - schema v3+ 演进
>
> **架构定位**：B 模块（底层逻辑 + 技术实现）剩余 phase 整合 master。base main HEAD = `34019c2464ed2461cc1b359cd78ad50769730abc`（B.1 PR #86 squash merge `8b17901f945efa381fe8044ea49ef1b83b397058` + master spec v1.0 docs commit + format fix）。
>
> **v1.1 修订原因**：v1.0 第 1 轮评估 5 类 P1 mismatch（角色模型 11→12 错 / 文件路径 / B.4 冻结矛盾 / B.7 endpoint 已存在 / B.8 跟单 UI 范围）。Dan 拍板 B.2 = 7→12 真升级；F 自决 B.4 narrow cron exception / B.7 新建 decision-history / B.8 narrow v10 exception / B.6 复用 stream / B.9 加 manual_close_requested reason。
...
- B.2 后：watch board 决策由 12 真 AI 角色产出（5 个 synthetic 升级为真 LLM call + 真 prompt + 真 rationale，含 memory_loop 真复盘主管）
...
### US-B2 — 12 真 AI 角色决策（P1）

**作为** 看 watch board 的访客
**我希望** 看到完整 12 真 AI 角色（5 个之前 synthetic 角色升级为真 LLM call + 真 rationale，含 memory_loop 真复盘主管）
**这样** 决策来源跟真实 team registry 对齐，不是"幽灵 5 个角色"
...
**Given** B.2 merge 后，5 个 synthetic 角色升级 real（含 LLM prompt / role / desc / provider）
**When** pmDecisionPipeline 跑一次完整 decision
**Then** record `analystInputs` 含 12 条 real rationale + `contributorIds` 含 12 个 TeamMemberId + memory_loop 真参与 + UI flow stage-6 显示 memory_loop 真 reflection
...
1. base main HEAD = `34019c2464ed2461cc1b359cd78ad50769730abc`（B.1 PR #86 squash `8b17901f` + master spec docs commit `7b849e68` + format fix `34019c24`）
2. **TeamMemberId 当前是 7 个**（`fundamental / news / chart / onchain / research_lead / risk_lead / pm`），不是 11
3. **DispatchAgentId 当前是 12 个**（11 个 dispatch role + `memory_loop` synthetic）
4. **跟单 UI 实际在两处**：`src/modules/agent-watch/v9/TopicStrategy.tsx`（B owned）+ `src/modules/agent-watch/v10/MarketAnalysisPanel.tsx`（A1 owned，B.8 narrow exception 允许）
5. `DecisionOutcome` schema 已含 `manual_close`，但 writer 当前不产
6. `DecisionResolutionReason` 当前仅 3 个值（`take_profit_reached / stop_loss_reached / evaluation_window_elapsed`），B.9 加 `manual_close_requested`
7. `/api/watch/history/route.ts` 已存在（返 public timeline），B.7 不动它；新建 `/api/watch/decision-history`
8. `/api/watch/stream/route.ts` 已存在（debug SSE），B.6 复用扩展而非新建
9. KV 是 Vercel `@vercel/kv` / Upstash Redis 风格，**不支持 `If-Match` / ETag**；B.4 CAS 用 version field + atomic ops
10. `pmDecisionPipeline.ts` 当前直接生成 `schemaVersion: 1` record + `analystInputs` + `stageTrace`；B.3 改这里
11. B.1 i18n outcome / writer 冻结 audit 已 PASS（PR #86 verified）
12. A1 v10 module + AgentWatchBoard console seam 稳定（B 全程不动除 B.8 narrow exception）
13. v9 视觉权威不变（B 全程不动 v9 组件树 CSS / layout）
14. 双 prod 双线锁定（Tier 1 ai.coinw.com / Tier 2 claw42.ai）—— B 全程仅 staging preview
15. CoinW API live trading 由独立 legal/security/API spec 处理（B.8 mock 解锁后下轮做）
16. i18n 10 locale + en_XA 全程维护（每 phase 新 namespace 全翻译）
17. team registry / TeamMember type 在 `src/lib/team/teamRegistry.ts`；`src/lib/team/types.ts` 不存在
18. dispatchAgentMapping 在 `src/lib/watch/dispatchAgentMapping.ts`（不是 v9/）
19. Codex 长时间运行期间 Vercel preview quota / GitHub Actions quota 充足
20. F session 不在 Codex 运行期间介入（除非 Codex 报停）
...
- `DecisionResolutionResult = Exclude<DecisionOutcome, null | "manual_close">`（writer return 不含 manual_close）
- `DecisionResolutionReason = "take_profit_reached" | "stop_loss_reached" | "evaluation_window_elapsed"`（不含 manual reason）
- `PublicTimelineEvent.pm_decision.resolution` 5 子字段
- `publicTimelineProjection.resolutionFromRecord`
- `TeamMemberId = "fundamental" | "news" | "chart" | "onchain" | "research_lead" | "risk_lead" | "pm"`（**7 个**）
- `DispatchAgentId = 12 个`（11 dispatch role + memory_loop synthetic）

**文件位置（v1.0 spec 路径错误已修正）**：

- TeamMemberId type + team registry：`src/lib/team/teamRegistry.ts`（**不是 `src/lib/team/types.ts`**）
...
- **B.2**: 7 TeamMember → 12 真升级（5 个新 TeamMember 加 LLM prompt / role / desc / provider；memory_loop 同步升级真 TeamMember）；dispatchAgentMapping 12→12 真映射；teamRegistry 加 5 个 entry
...
- **B.9**: writer manual_close 路径 + `DecisionResolutionReason` 加 `manual_close_requested` + admin API + adapter / outcome i18n 兼容
...
### 7.1 B.2 — 7 TeamMember → 12 真升级

**当前 state**：

- `TeamMemberId` 7 个真实 union（`fundamental / news / chart / onchain / research_lead / risk_lead / pm`）
- `DispatchAgentId` 12 个 union（11 dispatch + memory_loop synthetic）
- `dispatchAgentMapping.ts`: 7 TeamMember → 11 DispatchAgent + memory_loop synthetic
- `DISPATCH_AGENT_NOT_IN_CURRENT = ["bullish_researcher", "bearish_researcher", "trader", "aggressive_reviewer", "neutral_reviewer", "conservative_reviewer", "memory_loop"]`（synthetic 列表）

**B.2 目标 state**：

- `TeamMemberId` 升级为 12 个 union：原 7 + 5 新真 TeamMember (`bullish_researcher / bearish_researcher / trader / aggressive_reviewer / neutral_reviewer` —— 或按 Codex 实测的实际 synthetic 列表选 5 个) + `memory_loop`
- 注：原 synthetic 列表 7 个含 `conservative_reviewer`；F 选 5 个 + memory_loop = 6 加 7 = 13。Codex 实施时按 grep 实测 + 决定具体 5/6 个升级（如 reviewer 三选二 / bullish + bearish + trader 三个必升）。Codex 自决具体 5-6 个升级目标，但**目标总数必须 = 12**（dispatch 12 与 team 12 一致）
- `dispatchAgentMapping` 12→12 真映射 + `DISPATCH_AGENT_NOT_IN_CURRENT` 仅剩还在 phase 6a 不升级的（如全升则空数组）
- team registry 加 5-6 个新 entry（含 display name / role / desc / promptVersion / provider）
- `pmDecisionPipeline.ts` 调用 5-6 个新 LLM provider 跑 analystInputs
- `publicTimelineProjection` 处理 12 角色 record（不再 synthetic placeholder）

**关键决策**：

- F 自决：B.2 升级 6 个 synthetic 为 real（5 个 + memory_loop = 12 total），保留可能 1-2 个 synthetic 留 phase 6a
- Codex 实施时按 grep 实测 synthetic 列表 + decide 具体 5-6 个升级目标，遇模糊 → [SPEC-FEEDBACK] stop

**v9 hero 边界**：

- v9 hero constellation 当前 11 anode 无 a-mem node → B.2 不改 v9 视觉权威（hero 仍 11 anode）+ memory_loop 仅 flow stage-6 + chat 渲染（v9 已 a-mem CSS ready）
- v10 hero 自决（A1 owned，B 不动）

### 7.2 B.3 — Multi-round debate trace schema v2

**当前 state**：

- `StrategyDecisionRecord.schemaVersion: 1`
...
### 7.8 B.9 — manual_close real writer writeback（**显式解冻 B.1 冻结的 writer 文件 + 加 reason union**）

**当前 state**：

- `DecisionResolutionResult` writer return union 不含 `manual_close`（writer 当前只产 hit_tp / hit_sl / expired）
- `DecisionResolutionReason` 当前 3 个值（不含 manual reason）
...
- `DecisionResolutionResult` 加 `manual_close`
- `DecisionResolutionReason` 加 `"manual_close_requested"`（新 union 值）
...
- `src/lib/team/teamRegistry.ts`（**type + registry 都在这里**，加 5-6 个新 TeamMember entry + memory_loop entry + union 升级到 12）
- `src/lib/watch/dispatchAgentMapping.ts`（**不在 v9/** —— 重构 12→12 真映射 + `DISPATCH_AGENT_NOT_IN_CURRENT` 缩短）
- `src/lib/watch/publicTimelineProjection.ts`（处理 12 角色 record）
- `pmDecisionPipeline.ts`（含 5-6 个新 LLM provider call）
- LLM prompt 文件（按 main `src/lib/team/` prompt 结构）

**B.3**：

- `src/lib/team/teamRegistry.ts` 加 `schemaVersion: 2` 相关 types（`AnalystInputRoundRecord` / `DispatchStageRoundRecord`）
...
- `src/lib/team/teamRegistry.ts`（`DecisionResolutionResult` 加 `manual_close` + `DecisionResolutionReason` 加 `manual_close_requested`）
...
- [ ] T-B2-011 自决 5-6 个 synthetic → real 升级目标（dispatch 12 = team 12，必含 memory_loop）+ 报 F 在 codex-to-claude.md 写自决方案
- [ ] T-B2-012 `src/lib/team/teamRegistry.ts` 加 5-6 个新 TeamMember entry + memory_loop（display name / role / desc / promptVersion / provider）+ TeamMemberId union 升级到 12
- [ ] T-B2-013 LLM prompt 文件：5-6 个新角色 prompt（按 main `src/lib/team/` prompt 结构）
- [ ] T-B2-014 `src/lib/watch/dispatchAgentMapping.ts` 12→12 真映射 + `DISPATCH_AGENT_NOT_IN_CURRENT` 缩短
- [ ] T-B2-015 `pmDecisionPipeline.ts` 加 5-6 个新 LLM provider call（parallel + timeout 30s + retry 2x + fallback partial decision）
- [ ] T-B2-016 `src/lib/watch/publicTimelineProjection.ts` 处理 12 角色 record
- [ ] T-B2-017 i18n 微调（已有 dispatchV10.roles.\* 12 个；仅校准新 5-6 个角色 desc / stat 准确性）
...
- [ ] T-B3-011 `src/lib/team/teamRegistry.ts` 加 `schemaVersion: 2` types + `AnalystInputRoundRecord` / `DispatchStageRoundRecord`
- [ ] T-B3-012 schemaVersion union 从 `1` → `1 | 2`
...
- [ ] T-B9-011 `src/lib/team/teamRegistry.ts`（type 在这里）`DecisionResolutionResult` union 加 `manual_close` + `DecisionResolutionReason` 加 `manual_close_requested`
...
| Rule 4 数据 schema 纪律    | CONDITIONAL | B.3 升级 schema v1→v2 + B.4 加 partial state + B.9 writer union 加 manual_close + reason union 加 manual_close_requested —— 每个改动都在 spec 显式声明 + dual-shape 兼容 + decision-log 联动                       |
...
| 觉得 v1.0 → v1.1 修正了路径 / 命名 / 范围，实施时凭记忆继续用 v1.0 旧路径 | v1.1 是新基准，v1.0 旧文字作废                                                                            | Codex 实施前 cat 实测 + 按 v1.1 § 18 共通 grep check 走完                       |

---

## 18. 双脑判断点（每 phase 必填，Codex 实施前 cat 实测 + 写 codex-to-claude.md）

### Q-B2.1: team registry 当前 shape + 5-6 个 synthetic 升级目标自决

- `cat src/lib/team/teamRegistry.ts` 看 7 角色 entry 结构
- `cat src/lib/watch/dispatchAgentMapping.ts` 看 11 dispatch + memory_loop synthetic 映射
- grep `DISPATCH_AGENT_NOT_IN_CURRENT` 实际 list
- F 自决 5-6 个升级（必含 memory_loop，dispatch 12 = team 12，留 0-1 个 synthetic 给 phase 6a）
- LLM provider 配置如何加（按 main 现有 provider 结构）
- 给出最小改动方案 + risk + 加哪些 prompt 文件

### Q-B3.1: schema v1 → v2 dual-shape 兼容 + pmDecisionPipeline 改造

- `cat src/lib/team/teamRegistry.ts` 看当前 schema
...
### Q-B9.1: writer manual_close 路径 + reason union 扩展 + admin auth

- `cat src/lib/team/decisionResolution.ts` 看 DecisionResolutionResult union + writer flow
- `cat src/lib/team/teamRegistry.ts` 看 DecisionResolutionReason union 加 manual_close_requested 影响
...
- [ ] `cat src/lib/team/teamRegistry.ts | grep -A 20 TeamMemberId` 当前 union（**不是 `src/lib/team/types.ts` —— 该文件不存在**）
...
_v1.1 起草：2026-05-15 00:30 CST F session_
_起草前置：Codex v1.0 Round 1 5 类 P1 mismatch 实测数据 + Dan 拍 B.2 = 7→12 真升级 + F 自决 B.4 narrow cron exception / B.7 新建 decision-history / B.8 narrow v10 exception / B.6 复用 stream / B.9 加 manual_close_requested reason_
_下一动作：派 Codex 第 2 轮双脑评估（目标 ≤ 2 minor → 自动串行进入 B.2 → ... → B.9）_
