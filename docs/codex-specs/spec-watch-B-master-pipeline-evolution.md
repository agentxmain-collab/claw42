...
1. base main HEAD = `3173878566545c5303a2810235d4e3e41cd3452e`（v1.2 spec docs `b1d9043` + format fix `3173878`）；v1.3 spec docs commit 由 Codex T000 推进
...
6. **Schema types 文件归属（v1.2 实测修正）**：
   ...
   - `MarketDataSource` 当前 union 在 **`src/modules/agent-watch/types.ts`**（不在 `src/lib/team/` 下），当前值 = `"coinw-kline" | "coingecko-ticker" | "fallback"`
...
## 6. 现状盘点（B.1 PR #86 merge 后 v1.2 spec docs commit + format fix main HEAD `3173878566545c5303a2810235d4e3e41cd3452e`）
...
- `MarketDataSource = "coinw-kline" | "coingecko-ticker" | "fallback"` —— **in `src/modules/agent-watch/types.ts`**（不在 `src/lib/team/strategyDecisionRecord.ts`）—— B.9 加 `"admin_manual"`
...
- `MarketDataSource` 加 `"admin_manual"`（admin API 手动平仓 价格来源）—— 改 **`src/modules/agent-watch/types.ts`**
...
```
[base: B.1 PR #86 merge + v1.2 spec docs commit + format fix = 3173878]
...
StrategyDecisionRecord (B.3 schema v2 + rounds[])                             │
   ...
   ↑ B.2 team registry 14 real members + 7 个新 LLM provider call
   ↓
...
- v1.2 spec docs commit `b1d9043` + format fix `3173878` 已推 origin/main ✓
...
- [ ] T-B2-001 worktree `/tmp/claw42-b2-roles-upgrade` + branch `feature/spec-b-master-b2-roles-upgrade` base `3173878`
...
- [ ] T-B2-022 测试：pmDecisionPipeline mock 14 角色 LLM call + fallback case
- [ ] T-B2-030 staging fixture 加 1 条 14 角色完整 record
- [ ] T-B2-040 verify gate 全过 + Vercel preview READY
- [ ] T-B2-041 视觉 verify：hero 仍 11 anode（v10 不动）+ flow stage-6 + chat 14 角色完整渲染
...
- [ ] T-B3-042 grep gate：writer + cron 文件 0 diff（schema type 加在 `strategyDecisionRecord.ts`，cron 不动）
...
- [ ] T-B9-011b `src/lib/team/strategyDecisionRecord.ts` 加 `DecisionResolutionReason` `manual_close_requested`（DecisionResolutionReason 在这里）
- [ ] T-B9-011b2 `src/modules/agent-watch/types.ts` 加 `MarketDataSource` `"admin_manual"`（**MarketDataSource 实际在这里，不在 team/ 下**）
...
- 每 phase ≤ 5 commit / ≤ 3 AI 天（B.2 例外 ≤ 7 AI 天因为 7 个新 LLM 角色），超 → 停 + 报 F
...
- **base HEAD**: v1.2 spec docs + format fix `3173878566545c5303a2810235d4e3e41cd3452e`（B.1 PR #86 squash `8b17901f` + v1.2 spec docs `b1d9043` + format fix `3173878`）；v1.3 spec docs commit 由 Codex T000 推进时拿新 sha
...
- [ ] `cat src/lib/team/strategyDecisionRecord.ts` 看 schema types：StrategyDecisionRecord / AnalystInputRecord / DecisionResolutionReason / DecisionOutcome（**schema types 在这里**）
- [ ] `cat src/modules/agent-watch/types.ts` 看 MarketDataSource union（**MarketDataSource 在这里，不在 team/ 下**）
